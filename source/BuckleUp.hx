...
class BuckleUpShader extends FlxShader
{
  @:glFragmentSource('
        ...
        {
            vec3 ta = flixel_texture2D(channel, p.xy).xyz;
            vec3 tb = flixel_texture2D(channel, p.xz).xyz;
            vec3 tc = flixel_texture2D(channel, p.yz).xyz;
        ...
        void main( out vec4 fragColor)
        {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                vec3 stex = textex(bitmap, sw * 0.5);
                sk += stex * fr * 0.25;
            }
            ...
            prod = pow(prod, 4.0);
            vec3 pcol = textex(bitmap, w * 0.1) * prod;
